...
# Gets the response of  url that returns plain text as response.
def getPlainResponse(url):
    return urllib.request.urlopen(url).read().decode().strip()


# Gets the response of  url that returns json as response and replaces the default argument '{ip address}' with the IP address whose country we're looking
